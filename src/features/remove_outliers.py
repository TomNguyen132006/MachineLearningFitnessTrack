...
df[outlier[:3] +["label"]].boxplot(by = "label", figsize =(20,10), layout= (1,3))
df[outlier[3:] +["label"]].boxplot(by = "label", figsize =(20,10), layout= (1,3))
...
df[outlier[:3] +["label"]].plot.hist(by = "label", figsize =(20,20), layout= (3,3))
df[outlier[3:] +["label"]].plot.hist(by = "label", figsize =(20,20), layout= (3,3))
...
# Loop over all columns
dataset, outliers, X_scores =  mark_outliers_lof(df, outlier)
for col in outlier:
...
dataset, outliers, X_scores =  mark_outliers_lof(df[df["label"] == label], outlier)
for col in outlier:
    plot_binary_outliers(dataset = dataset, col=col, outlier_col = "outlier_lof", reset_index = True)
...
for col in outlier:
    for label in df["label"].unique():
        ...
        removed_df.loc[(removed_df["label"] == label), col] = dataset[col]
        
        n_ouliers = len(df) - len(removed_df[col].dropna())
